...
class InternalIdentityHeaderMiddleware(IdentityRequest):
    ...
    def test_s2s_can_be_accessed_through_psk(self):
        ...
        request = self.request_context["request"]
        client = APIClient()
        response = client.post(f"/_private/_s2s/workspaces/{self.org_id}/ungrouped/", **request.META)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        response = client.get(f"/_private/_s2s/workspaces/{self.org_id}/ungrouped/", **self.service_headers)
