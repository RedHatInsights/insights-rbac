...
class WorkspaceViewTests(IdentityRequest):
    ...
    def setUp(self):
        ...
        self.standard_sub_workspace = Workspace.objects.create(
...
class WorkspaceMove(WorkspaceViewTests):
    ...
    def test_move_under_standard_workspace(self):
        ...
        # Move the workspace under standard workspace
    ...
    def test_move_enforce_hierarchy_depth(self):
        ...
        # Create new workspace under default workspace
        client = APIClient()
        workspace_data = {"name": "New Workspace", "description": "Workspace"}
        url = reverse("v2_management:workspace-list")
    ...
    def test_move_enforce_hierarchy_depth_with_descendants_success(self):
        ...
        # Create new workspace under default workspace
        client = APIClient()
        workspace_C_data = {"name": "New Workspace C", "description": "Workspace C"}
        url = reverse("v2_management:workspace-list")
        response = client.post(url, workspace_C_data, format="json", **self.headers)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        workspace_id = response.data.get("id")
        workspace_C = Workspace.objects.get(id=workspace_id)

        # Create a child workspace
        workspace_D_data = {"name": "New Workspace D", "description": "Workspace D", "parent_id": workspace_C.id}
        url = reverse("v2_management:workspace-list")
        response = client.post(url, workspace_D_data, format="json", **self.headers)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_move_under_descendant(self):
        ...
        for name in "ABCDE":
            ...
            workspace_id = response.data.get("id")
            workspaces[name] = Workspace.objects.get(id=workspace_id)
    ...
    def test_move_with_duplicate_name_under_target_parent(self):
        ...
        client = APIClient()
        test_workspace_data = {"name": name, "description": f"{name} description"}
        url = reverse("v2_management:workspace-list")
        response = client.post(url, test_workspace_data, format="json", **self.headers)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        workspace_id = response.data.get("id")
        test_workspace = Workspace.objects.get(id=workspace_id)
