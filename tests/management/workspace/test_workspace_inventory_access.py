...
class WorkspaceInventoryAccessV2Tests(IdentityRequest):
    ...
    def test_workspace_list_non_org_admin_default_group(self, mock_flag, mock_channel):
        ...
        with patch.object(
            ...
            "MessageToDict",
            side_effect=[
                {"object": {"resourceId": str(self.default_workspace.id)}},
                {"object": {"resourceId": str(self.standard_workspace.id)}},
            ],
        ):
            with patch(
    ...
    def test_workspace_list_custom_group_without_attribute_filter(
        ...
        with patch.object(
            ...
            "MessageToDict",
            side_effect=[
                {"object": {"resourceId": str(self.root_workspace.id)}},
                {"object": {"resourceId": str(self.default_workspace.id)}},
                {"object": {"resourceId": str(self.standard_workspace.id)}},
                {"object": {"resourceId": str(self.standard_sub_workspace.id)}},
            ],
        ):
            with patch(
                "management.permissions.workspace_inventory_access.inventory_service_pb2_grpc.KesselInventoryServiceStub",
