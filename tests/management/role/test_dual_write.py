...
class SeedingRelationApiDualWriteHandlerTest(DualWriteTestCase):
    """Test dual write logic for system roles using the seeding handler."""
    ...
    def test_replicate_new_system_role(self):
        """Test replication of a new system role."""
        # Create a new system role
        permissions = ["inventory:hosts:read", "inventory:hosts:write"]
        role = self.given_v1_system_role("new_role", permissions=permissions, platform_default=True)

        # Initialize the handler
        handler = SeedingRelationApiDualWriteHandler(role)

        # Track the calls to the replicator
        current_relations = []
        with patch.object(handler._replicator, "replicate") as mock_replicate:
            # Trigger replication
            handler.replicate_new_system_role()

            # Verify the replication event
            mock_replicate.assert_called_once()
            call_args = mock_replicate.call_args[0][0]

            # Verify event type
            self.assertEqual(call_args.event_type, ReplicationEventType.CREATE_SYSTEM_ROLE)

            # Verify metadata
            self.assertEqual(call_args.info["role_uuid"], role.uuid)

            # Verify relationships (should have permissions and platform_default binding)
            self.assertEqual(len(call_args.remove), 0)  # No relations to remove for new role
            self.assertGreater(len(call_args.add), 0)  # Should have new relations

            # Verify the added relations contain our permissions
            relations = call_args.add
            permission_relations = [r for r in relations if r.relation.name == "permission"]
            self.assertEqual(len(permission_relations), 2)  # One for each permission

    def test_replicate_deleted_system_role(self):
        """Test replication of system role deletion."""
        # Create a system role that will be deleted
        permissions = ["inventory:hosts:read"]
        role = self.given_v1_system_role("delete_role", permissions=permissions)

        # Initialize the handler
        handler = SeedingRelationApiDualWriteHandler(role)

        # Track current relations before deletion
        handler.prepare_for_update()  # This generates current relations

        # Track the calls to the replicator
        with patch.object(handler._replicator, "replicate") as mock_replicate:
            # Trigger replication of deletion
            handler.replicate_deleted_system_role()
            ...
            self.assertEqual(len(call_args.add), 0)  # No new relations for deletion

            # Verify the removed relations match our permissions
            permission_relations = [r for r in call_args.remove if r.relation.name == "permission"]
            self.assertEqual(len(permission_relations), 1)  # One permission to remove

    def test_replicate_update_system_role(self):
        """Test replication of system role updates."""
        # Create initial system role
        initial_permissions = ["inventory:hosts:read"]
        role = self.given_v1_system_role("update_role", permissions=initial_permissions)

        # Initialize the handler
        handler = SeedingRelationApiDualWriteHandler(role)

        # Track current state before update
        handler.prepare_for_update()

        # Update the role with new permissions
        updated_permissions = ["inventory:hosts:read", "inventory:hosts:write"]
        self.given_update_to_v1_role(role, default=updated_permissions)

        # Track the calls to the replicator
        with patch.object(handler._replicator, "replicate") as mock_replicate:
            # Trigger replication of update
            ...
            # Verify event type
            self.assertEqual(call_args.event_type, ReplicationEventType.UPDATE_SYSTEM_ROLE)

            # Verify metadata
            self.assertEqual(call_args.info["role_uuid"], role.uuid)

            # Verify relationships
            self.assertGreater(len(call_args.remove), 0)  # Should have old relations to remove
            self.assertGreater(len(call_args.add), 0)  # Should have new relations to add

            # Verify the relations reflect our permission changes
            removed_perms = [r for r in call_args.remove if r.relation.name == "permission"]
            added_perms = [r for r in call_args.add if r.relation.name == "permission"]

            self.assertEqual(len(removed_perms), 1)  # One old permission removed
            self.assertEqual(len(added_perms), 2)  # Two new permissions added
