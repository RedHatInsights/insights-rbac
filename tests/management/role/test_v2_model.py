...
class RoleV2ModelTests(IdentityRequest):
    ...
    def test_rolev2_creation(self):
        ...
        self.assertEqual(role.display_name, "Test role display name")
        self.assertEqual(role.tenant, self.tenant)
        self.assertTrue(role.uuid)  # UUID exists and is auto-generated

    def test_rolev2_display_name_auto_population(self):
        """Test that display_name is auto-populated from name."""
        role = RoleV2.objects.create(name="test_role", tenant=self.tenant)
        self.assertEqual(role.display_name, "test_role")
    ...
    def test_rolev2_unique_display_name_per_tenant(self):
        """Test unique display name per tenant."""
        RoleV2.objects.create(name="test_role", display_name="display_name", tenant=self.tenant)
        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                RoleV2.objects.create(name="test_role_2", display_name="display_name", tenant=self.tenant)
    ...
    def test_custom_role_creation_with_wrong_type(self):
        """Test V2 Custom Role creation with wrong type causes the type to be set to custom."""
        role = CustomRoleV2.objects.create(name="custom_role", type=RoleV2.Types.PLATFORM, tenant=self.tenant)
        self.assertEqual(role.type, RoleV2.Types.CUSTOM)
    ...
    def test_seeded_role_creation_with_wrong_type(self):
        """Test V2 Seeded Role creation with wrong type causes the type to be set to seeded."""
        role = SeededRoleV2.objects.create(name="seeded_role", type=RoleV2.Types.CUSTOM, tenant=self.tenant)
        self.assertEqual(role.type, RoleV2.Types.SEEDED)
    ...
    def test_platform_role_creation_with_wrong_type(self):
        """Test V2 Platform Role creation with wrong type causes the type to be set to platform."""
        role = PlatformRoleV2.objects.create(name="platform_role", type=RoleV2.Types.SEEDED, tenant=self.tenant)
        self.assertEqual(role.type, RoleV2.Types.PLATFORM)
