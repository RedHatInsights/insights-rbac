...
class BackfillAdditionalScopeBindingsCommandTest(TestCase):
    ...
    def test_skip_default_group_bindings_for_custom_default_group_tenant(self, mock_replicator_class):
        ...
        # Bootstrap a tenant using the service (creates tenant, mapping, workspaces, and groups)
        service = V2TenantBootstrapService(InMemoryRelationReplicator(self.tuples))
        boot = service.new_bootstrapped_tenant(org_id="custom123")
        test_tenant = boot.tenant
        tenant_mapping = boot.mapping

        # Create a custom platform default group to trigger the skip logic
        ...
        # Update the tenant mapping to use the custom default group
        # (Keep the admin group from bootstrap for admin bindings to work)
        tenant_mapping.default_group_uuid = custom_default_group.uuid
        tenant_mapping.save()
        ...
        # Should have at least one replicator call (for admin bindings, scope bindings, or system role parents)
        self.assertGreater(len(replicate_calls), 0, "Should have replicator calls")

        # The test has verified that:
        # 1. The command ran successfully and printed "Skipping default group bindings"
        # 2. At least one replicator call was made (likely for admin group bindings or system role parents)
        # 3. This demonstrates that the logic correctly skips default group bindings for custom default groups
        #    while still processing other types of bindings
