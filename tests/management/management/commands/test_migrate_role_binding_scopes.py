...
from migration_tool.in_memory_tuples import InMemoryTuples, InMemoryRelationReplicator, all_of, resource, relation
...
class TestMigrateRoleBindingScopesCommand(TestCase):
    ...
    def test_migrate_default_workspace_binding_to_root(self, mock_outbox_replicator):
        ...
        with patch(
            "management.management.commands.migrate_role_binding_scopes.highest_scope_for_permissions"
    ...
    def test_migrate_default_workspace_binding_to_tenant(self, mock_outbox_replicator):
        ...
        with (
            patch("management.permission_scope.APP_SCOPE_MAPPING", _build_app_scope_mapping()),
            patch(
                "management.management.commands.migrate_role_binding_scopes.highest_scope_for_permissions"
            ) as mock_highest_scope,
        ):
    ...
    def test_keep_default_workspace_binding_for_default_scope_permissions(self, mock_outbox_replicator):
        ...
        with (
            ...
            # Create a binding mapping currently bound to default workspace using proper factory method
            from migration_tool.models import V2role, V2boundresource, V2rolebinding
    ...
    def test_mixed_permission_scopes_uses_highest_scope(self, mock_outbox_replicator):
        ...
        with (
            ...
            v2_role = V2role(
                id=str(self.root_scope_role.uuid),
                is_system=True,
                permissions=frozenset(["inventory:groups:read", "advisor:recommendation:read", "patch:system:read"]),
            )
    ...
    def test_binding_without_permissions_kept_default(self, mock_outbox_replicator):
        ...
        with (
            ...
            # Create a binding mapping with no permissions (system role with no Access objects) using proper factory method
            from migration_tool.models import V2role, V2boundresource, V2rolebinding
    ...
    def test_custom_role_with_mappings_migrated_to_root(self, mock_outbox_replicator):
        ...
        with (
            patch("management.permission_scope.APP_SCOPE_MAPPING", _build_app_scope_mapping()),
            patch(
                "management.management.commands.migrate_role_binding_scopes.highest_scope_for_v2_permissions"
            ...
            # Create a binding mapping for custom role with root-level permissions using proper factory method
            from migration_tool.models import V2role, V2boundresource, V2rolebinding
    ...
    def test_batch_processing_efficiency(self, mock_outbox_replicator):
        ...
        with (
            ...
            for i, binding in enumerate(bindings):
                ...
                expected_tenant = expected_tenants[i]
