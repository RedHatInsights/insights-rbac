...
from api.models import Access, Permission, ResourceDefinition, Role
...
def _make_role(data):
    ...
    if created:
        if role.display_name != display_name:
            ...
            role.save()
        logger.info("Created role %s for public schema.", name)
    else:
        if role.version != defaults["version"]:
            Role.objects.filter(name=name).update(**defaults, display_name=display_name)
            logger.info("Updated role %s for public schema.", name)
            role.access.all().delete()
        else:
            logger.info("No change in role %s for public schema.", name)
            return role
...
def _update_or_create_roles(roles):
    ...
    for role_json in roles:
        try:
            _make_role(role_json)
        except Exception as e:
            logger.error(
                f"Failed to update or create role: {role_json.get('name')} " f"for public schema with error: {e}"
            )


def seed_roles():
    """For a schema update or create system defined roles."""
    ...
    with transaction.atomic():
        for role_file_name in role_files:
            ...
            with open(role_file_path) as json_file:
                ...
                _update_or_create_roles(role_list)


def seed_permissions():
    """For public schema update or create defined permissions."""
                ...
                for resource, operation_objects in data.items():
                    try:
                        for operation_object in operation_objects:
                            ...
                            if created:
                                logger.info(f"Created permission {permission.permission} " f"for public schema.")
                                # current_permission_ids.add(permission.id)
                    except Exception as e:
                        logger.error(
                            f"Failed to update or create permissions for: "
                            f"{app_name}:{resource} for public schema with error: {e}"
                        )
