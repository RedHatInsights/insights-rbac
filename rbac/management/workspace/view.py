...
class WorkspaceViewSet(WorkspaceObjectAccessMixin, BaseV2ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Validate and filter by id parameter (comma-separated list of UUIDs)
        if id_filter is not None:
            if not id_filter.strip():
                id_filter = None  # Treat empty id as unset, returning all results
            else:
                if "\x00" in id_filter:
                    raise serializers.ValidationError({"id": "The 'id' query parameter contains invalid characters."})

                ids = list(dict.fromkeys(id_val.strip().lower() for id_val in id_filter.split(",") if id_val.strip()))
