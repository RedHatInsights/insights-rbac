...
@register_tool(
    description=(
        ...
        "TROUBLESHOOTING: To find all V2 role bindings that effectively grant access to "
        "a specific user (including bindings inherited through group membership), use "
        "granted_subject_type='user' and granted_subject_id='<user_id>'. This is the "
        "V2 equivalent of list_access(username=X) for V1. "
        "ACCESS RESOLUTION: To check whether a user has a specific permission in V2, "
        "(1) call list_role_bindings(granted_subject_type='user', granted_subject_id='<user_id>') "
        "to find their effective bindings, (2) for each binding extract the role UUID, "
        "(3) call get_role(role_uuid=...) or list_role_access(role_uuid=...) to inspect "
        "the role's permissions, (4) check if the target permission is present. "
...
@register_tool(
    description=(
        ...
        "TROUBLESHOOTING: This is the fastest way to answer 'Can user X do Y?' in V1. "
        "For V2 applications, use list_role_bindings with "
        "granted_subject_type='user' and granted_subject_id='<user_id>' to find effective "
        "bindings (including group membership), then call get_role or list_role_access on "
        "each bound role to check its permissions. "
