...
def spicedb_relationships(v2_role_bindings: FrozenSet[V2rolebinding], root_workspace: Workspace):
    ...
    for v2_role_binding in v2_role_bindings:
        ...
        for bound_resource in v2_role_binding.resources:
            ...
            if not (
                bound_resource.resource_type == "workspace" and bound_resource.resourceId == str(root_workspace.uuid)
            ):
                relationships.append(
                    create_relationship(
...
def migrate_role(role: Role, write_db: bool, root_workspace: Workspace, default_workspace: Workspace):
...
def migrate_users(tenant: Tenant, write_db: bool):
    ...
    relationships = [
        create_relationship("user", str(principal.uuid), "tenant", str(tenant.org_id), "orgization")
...
def migrate_users_for_groups(tenant: Tenant, write_db: bool):
    ...
    for group in tenant.group_set.all():
        ...
        user_set = tenant.principal_set.all() if group.platform_default else group.principals.all()
