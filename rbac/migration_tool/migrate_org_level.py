...
#
"""Contains utilities for migrating existing roles to org-level permissions."""
...
def migrate_seeded_to_org_level(
    replicator: Optional[RelationReplicator], resource_service: Optional[ImplicitResourceService]
):
    """
    ...
    # Removing relations happens before adding them, so having the same relation in both list as fine,
    # and the relation will end up being present. This means we can remove the existing relations without worrying
    # about whether we'll immediately add them back.
