...
def run_zed_check(resource_type, resource_id, permission, subject_type, subject_id, subject_relation=None):
    """
    ...
    cmd = ["zed", "permission", "check", f"rbac/{resource_type}:{resource_id}", permission, subject]
...
def check_permission(workspace_id, permission, principal_id=None, group_id=None, verbose=False):
    """
    ...
    has_permission, output = run_zed_check(
        "workspace", workspace_id, permission, subject_type, subject_id, subject_relation
    )

    if has_permission:
        print(f"✓ GRANTED: {subject_display} has '{permission}' on workspace:{workspace_id}")
        if verbose:
            print(f"  {output.strip()}")
        return True
    else:
        print(f"✗ DENIED: {subject_display} does NOT have '{permission}' on workspace:{workspace_id}")
        if verbose:
            print(f"  {output.strip()}")
        return False
...
def main():
    ...
    parser.add_argument("--principal", help="Principal ID to check (e.g., localhost/username)")
    parser.add_argument("--group", help="Group UUID to check")
    parser.add_argument("--permission", help="Specific permission to check (e.g., inventory_host_view)")
    parser.add_argument("--check-inventory", action="store_true", help="Check all inventory host permissions")
    ...
    if args.check_inventory:
        permissions = ["inventory_host_view", "inventory_host_update", "inventory_host_delete", "inventory_host_move"]
    ...
    for perm in permissions:
        granted = check_permission(
            args.workspace, perm, principal_id=args.principal, group_id=args.group, verbose=args.verbose
        )
        results[perm] = granted

    # Summary
    print("\n" + "=" * 80)
    granted_count = sum(1 for v in results.values() if v)
