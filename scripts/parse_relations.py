...
def get_resource_name(resource_type, resource_id, name_cache, force_refresh=False, debug=False):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            if resource_type == "workspace" or resource_type == "rbac/workspace":
                cursor.execute("SELECT name FROM management_workspace WHERE id::text = %s", [resource_id])
                row = cursor.fetchone()
                if row:
                    name = row[0]
                elif debug:
                ...
                cursor.execute(
                    "SELECT COALESCE(display_name, name) FROM management_role WHERE uuid::text = %s", [resource_id]
                )
                row = cursor.fetchone()
                if row:
                    name = row[0]
                    ...
                    if row:
                        ...
                        if row:
                            ...
                            match = re.search(
                                r"[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", policy_name
                            )
                            if match:
            ...
            elif resource_type == "role_binding" or resource_type == "rbac/role_binding":
        ...
        if debug:
            print(f"[DEBUG] Error fetching name for {resource_type}:{resource_id}: {e}", file=sys.stderr)
        name_cache[cache_key] = None
        return None
...
def interactive_input_handler(all_relations, permission_cache, stop_event, render_trigger, auto_check_mode):
    """
    ...
    try:
        ...
        # If /dev/tty is not available, just exit
        pass
    finally:
        try:
            tty_input.close()
...
def build_graph_structure(relations):
    ...
    for rel in relations:
        ...
        if resource_node not in graph:
            graph[resource_node] = {"type": rel["resource_type"], "id": rel["resource_id"], "edges": []}

        # Add subject node
        if subject_node not in graph:
            graph[subject_node] = {"type": rel["subject_type"], "id": rel["subject_id"], "edges": []}
...
def render_graph(
    ...
    root_nodes = [node_id for node_id in graph.keys() if node_id not in all_targets]

    # If no clear roots, use nodes with outgoing edges as roots
    if not root_nodes:
        root_nodes = [node_id for node_id, node_data in graph.items() if node_data["edges"]]
    ...
    summary_parts = [f"Nodes: {num_nodes}", f"Edges: {num_edges}", f"Roots: {len(root_nodes)}"]
    ...
    if interactive and permission_cache:
        ...
        checked_count = sum(1 for idx in permission_cache.keys() if idx >= 0)
        if checked_count > 0:
            # Count how many exist vs don't exist
            exists_count = sum(
                1
                for idx, cache_entry in permission_cache.items()
                if idx >= 0 and len(cache_entry) >= 1 and cache_entry[0]
            )
            missing_count = sum(
                1
                for idx, cache_entry in permission_cache.items()
                if idx >= 0 and len(cache_entry) >= 1 and not cache_entry[0]
            )
...
def colorize_relation(relation_str, uuid_colors, enable_color=True, show_names=False, name_cache=None):
    ...
    for i, part in enumerate(parts):
        if ":" in part:
            ...
            if "#" in part:
                ...
                if id_part == "*":
                    colored_part = (
                        f"{Colors.BRIGHT_WHITE}{type_part}{Colors.RESET}:{Colors.RED}{id_part}{Colors.RESET}"
                    )
                else:
                    uuid_color = get_uuid_color(id_part, uuid_colors)
                    colored_part = (
                        f"{Colors.BRIGHT_WHITE}{type_part}{Colors.RESET}:{uuid_color}{id_part}{Colors.RESET}"
                    )
...
def parse_relation(line):
    ...
    pattern = r"(\w+):([a-zA-Z0-9\-\*\/\.]+)#(\w+)@(\w+):([a-zA-Z0-9\-\*\/\.]+)(?:#(\w+))?"

    match = re.match(pattern, line.strip())
    if not match:
        return None

    resource_type, resource_id, relation, subject_type, subject_id, subject_relation = match.groups()
...
def check_permission_with_zed(relation_dict):
    ...
    try:
        ...
        return (has_permission, result.stdout.strip() if result.stdout else result.stderr.strip(), check_cmd)
    except subprocess.TimeoutExpired:
        return (False, "Timeout", check_cmd)
    except FileNotFoundError:
        return (False, "zed command not found", check_cmd)
    except Exception as e:
        return (False, str(e), check_cmd)
...
def extract_relations_from_line(line):
    ...
    if "INFO:" in line:
        ...
        if len(parts) == 2:
            result = parse_relation(parts[1].strip())
            if result:
                return result
...
def main():
    ...
    parser.add_argument(
        "--raw", action="store_true", help="Output raw relation format (default if --zed not specified)"
    )
    parser.add_argument("--quiet", action="store_true", help="Suppress header comments")
    parser.add_argument("--execute", action="store_true", help="Execute zed commands (requires --zed flag)")
    parser.add_argument("--verbose", action="store_true", help="Show which relations are being created/touched")
    parser.add_argument(
        "--compact",
        action="store_true",
    ...
    if args.show_names:
        if not setup_django():
            print("Warning: Failed to setup Django environment. Names will not be shown.", file=sys.stderr)
    ...
    if args.interactive and (args.visual or args.visual_static):
        input_thread = threading.Thread(
            ...
            args=(all_relations, permission_cache, stop_event, render_trigger, auto_check_mode),
            daemon=True,
        )
        input_thread.start()
    ...
    seeding_complete_markers = [r"Starting development server at", r"Starting principal updates"]
    ...
    try:
        while True:
            ...
            if in_startup_phase and any(re.search(marker, line) for marker in seeding_complete_markers):
            ...
            if args.filter_seeds and any(re.search(pattern, line) for pattern in seed_filter_patterns):
                continue
            ...
            if relation:
                ...
                if args.visual:
                    ...
                    if args.zed and args.execute:
                        ...
                        try:
                            result = subprocess.run(zed_cmd.split(), capture_output=True, text=True, check=True)
                            ...
                            print(f"\n✗ Error: 'zed' command not found. Is zed installed?", file=sys.stderr)
                            sys.exit(1)
                    ...
                    if args.execute:
                        ...
                        try:
                            ...
                            if args.compact:
                                ...
                                parts = zed_cmd.split()
                                if len(parts) >= 5:  # zed relationship touch resource relation subject
                            ...
                            print(f"✗ Error: 'zed' command not found. Is zed installed?", file=sys.stderr)
                            sys.exit(1)
                    else:
                        if args.compact:
                            # Compact mode: show just "resource relation subject"
